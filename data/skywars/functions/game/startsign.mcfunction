...
execute if entity @a[team=playing,nbt={Dimension:"skywars:skywars"}] run tellraw @s [{"text":"There's a game already in progress!","color":"red"}]
execute if entity @a[team=playing,nbt={Dimension:"skywars:skywars"}] run scoreboard players add #errors sw.dummy 1
